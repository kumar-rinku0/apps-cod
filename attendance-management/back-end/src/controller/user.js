...
import { generateRandomString, isRightUser } from "../util/fuctions.js";
...
const handleUserSignUpWithRoles = async (req, res) => {
  const { firstName, lastName, email, role, companyId, branchId } = req.body;
  const userbyemail = await User.findOne({ email });
  if (userbyemail) {
    const info = userbyemail.companyWithRole.find((item) => {
...
const handleGetUserByCompanyId = async (req, res) => {
  ...
  return res.status(400).send({ message: "don't have employee's yet!" });
};
//
//
//
